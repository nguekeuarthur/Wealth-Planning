        ...
        <div className="bg-white border border-[#dfe8e1] rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 p-6">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xs text-[#7a8b7f] mt-1 flex items-center gap-1">
                <FaEye className="text-xs" /> Visualisation
              </p>
            </div>
            <div className="p-3 rounded-xl bg-[#f0f5f1] text-[#2d5f3f] shadow-inner">
      ...
      <div className="my-10">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <h3 className="text-2xl font-bold text-[#1e4029]">Tous les projets</h3>
          </div>
          <button
            onClick={() => navigate("/collaborator/projects")}
          ...
          {stats.projects.length > 0 ? (
            stats.projects.slice(0, 3).map((project) => (
              <div
                ...
                onClick={() => navigate(`/collaborator/project/${project._id || project.id}`)}
              ...
              >
                ...
                <div className="h-2 bg-[#f4f7f4] rounded-full overflow-hidden">
                  <div
                    className="h-full rounded-full bg-[#5a8f3f]"
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <h3 className="text-2xl font-bold text-[#1e4029]">
                Factures (Mode visualisation)
              </h3>
            </div>
            <span className="text-xs text-[#7a8b7f] flex items-center gap-1">
              <FaEye className="text-xs" /> Lecture seule
            </span>
          </div>
          ...
          <div className="space-y-3">
            {stats.invoices.length > 0 ? (
              stats.invoices.slice(0, 5).map((invoice) => (
                <div
                  key={invoice._id}
                  className="flex items-center gap-4 p-4 bg-[#f4f7f4] rounded-xl"
                >
                  <FaFileInvoiceDollar className="text-[#2d5f3f] text-xl" />
                  <div className="flex-1">
                    <p className="font-medium text-[#1e4029]">
                      {invoice.invoiceNumber} - {invoice.service}
                    </p>
                    <p className="text-xs text-[#7a8b7f]">
                      {addThousandsSeparator(invoice.amount)} € • {invoice.status}
                    </p>
                  </div>
                </div>
              ))
              ...
              <div className="text-center py-8">
                ...
                <p className="text-[#7a8b7f]">Aucune facture</p>
              </div>
            )}
          </div>
        </div>
